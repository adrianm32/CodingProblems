...
private:
	...
	void T9Impl(_In_opt_ NODE *pTrieRoot, _In_ char *pT9, _In_ list<char *> *pSuggestions);
...
public:
	...
	list<char *> GetSpellingSuggestions(_In_ char *pWord, _In_ int nEditDistance = 1);

	list<char *> GetT9Suggestions(_In_ char *pT9);
...
list <char *> Trie::GetSpellingSuggestions(_In_ char *pWord, _In_ int nEditDistance)
{
...
char *ppDigitToCharsMap[] = {"+", "", "abc", "def", "ghi", "jkl", "mno", "pqrs", "tuv", "wxyz"};
void Trie::T9Impl(_In_opt_ NODE *pTrieRoot, _In_ char *pT9, _In_ list<char *> *pSuggestions)
{
	if (*pT9 == '\0' && pTrieRoot && pTrieRoot->bIsWord)
	{
		pSuggestions->push_back(pTrieRoot->pWord);
	}
	...
	int nDigit = *pT9 - '0';
	char *pDigitToChars = ppDigitToCharsMap[nDigit];
	int length = strlen(pDigitToChars);
	...
	for (int i = 0; i < length; i++)
	{
		...
		if (pCurr)
		{
			T9Impl(pCurr, &pT9[1], pSuggestions);
		}
	}
}


list<char *> Trie::GetT9Suggestions(_In_ char *pT9)
{
	list<char *> suggestions;

	T9Impl(this->Root, pT9, &suggestions);
...
void DisplaySuggestions(_In_ Trie *pTrie, _In_ char *pWord)
{
	...
	suggestions = pTrie->GetSpellingSuggestions(pWord, 1);
	DisplayList(&suggestions);

	printf_s("Suggestions with Edit Distance  = 2 : \n");
	suggestions = pTrie->GetSpellingSuggestions(pWord, 2);
	DisplayList(&suggestions);

	printf_s("Suggestions with Edit Distance  = 3 : \n");
	suggestions = pTrie->GetSpellingSuggestions(pWord, 3);
	DisplayList(&suggestions);
...
void DisplayT9Suggestions(_In_ Trie *pTrie, _In_ char *pT9)
{
	list<char *> suggestions;

	printf_s("\nInput T9 text: %s\n", pT9);
	suggestions = pTrie->GetT9Suggestions(pT9);
	DisplayList(&suggestions);

}
void TestT9PredictiveSuggestions()
{
	list<char *> suggestions;
	...
	DisplayT9Suggestions(pTrie, "43556");
	DisplayT9Suggestions(pTrie, "227");
	DisplayT9Suggestions(pTrie, "5683");
